 ...
 *  gcc c_tests/main_junoDCO.c lib/*.c -Ilib -lm -lportaudio -lportmidi -lcurses -o c_apps/main_junoDCO
 *
 * Compile on Windows with:
 *  gcc c_tests/main_junoDCO.c lib/*.c -Ilib -lm -lportaudio -lportmidi -lcurses -o c_apps/main_junoDCO.exe
 *
 * Run on linux and MacOS with:
 *  ./c_apps/main_junoDCO
 *
 * Run on Windows with:
 *  c_apps\main_junoDCO.exe
*/
...
struct DSP {
    ...
    struct junoDCO *voice[NUMBER_OF_CHANNELS];
...
struct DSP * dsp_init() {
    ...
    for (i = 0; i < NUMBER_OF_CHANNELS; i++) {
        ...
        dsp->voice[i] = junoDCO_init(FREQ, SAMPLE_RATE);
    }
    return dsp;
}
...
void dsp_process(const float *in, float *out, unsigned long framesPerBuffer, struct DSP *dsp) {
    ...
    for (i=0; i<framesPerBuffer; i++) {             /* For each sample frame in a buffer size... */
        for (j=0; j<NUMBER_OF_CHANNELS; j++) {      /* For each channel in a frame... */
            ...
            out[index] = junoDCO_process(dsp->voice[j], lfoIn);
...
void dsp_midi_note_in(struct DSP *dsp, int pitch, int velocity) {
    ...
    for (i = 0; i < NUMBER_OF_CHANNELS; i++) 
    {
        junoDCO_set_freq(dsp->voice[i], mtof(pitch)); 
    } 
}
