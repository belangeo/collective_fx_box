/*
 * test file to create a live audio processing program with portaudio, portmidi and ncurses to create a text interface.
 *
 * Compile on linux and MacOS with:
 *  gcc c_tests/main_test_midi_notes.c lib/sinosc.c lib/utils.c lib/rms.c lib/midimap.c -Ilib -lm -lportaudio -lportmidi -lcurses -o c_apps/main_test_midi_notes
 *
 * Compile on Windows with:
 *  gcc c_tests/main_test_midi_notes.c lib/sinosc.c lib/rms.c lib/midimap.c -Ilib -lm -lportaudio -lportmidi -lncurses -o c_apps/main_test_midi_notes.exe
 *
 * Run on linux and MacOS with:
 *  ./c_apps/main_test_midi_notes
 *
 * Run on Windows with:
 *  c_apps\main_test_midi_notes.exe
*/
...
struct DSP * dsp_init() {
    ...
    for (i = 0; i < NUMBER_OF_CHANNELS; i++) {
        ...
        dsp->osc[i] = sinosc_init(FREQ, SAMPLE_RATE);
    }
    return dsp;
}
...
void dsp_process(const float *in, float *out, unsigned long framesPerBuffer, struct DSP *dsp) {
    ...
    for (i=0; i<framesPerBuffer; i++) {             /* For each sample frame in a buffer size... */
        for (j=0; j<NUMBER_OF_CHANNELS; j++) {      /* For each channel in a frame... */
            ...
            out[index] = sinosc_process(dsp->osc[j]);
...
void dsp_midi_note_in(struct DSP *dsp, int pitch, int velocity) {
    ...
    int i;
    for (i = 0; i < NUMBER_OF_CHANNELS; i++) { 
        sinosc_set_freq(dsp->osc[0], mtof(pitch)); 
        sinosc_set_freq(dsp->osc[1], 1.005 * mtof(pitch));
    }
}
